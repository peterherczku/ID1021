...
void bench(int loop, int repeats, int max, int steps) {
    struct timespec t_start, t_stop;
    for(int i = 1; i < (max / steps); i++) {
        int size = i * steps;
        long min = LONG_MAX;
        for (int k = 0; k < loop; k++) {
            linked** listA = (linked**)malloc(repeats * sizeof(linked*));
            linked** listB = (linked**)malloc(repeats * sizeof(linked*));
            for (int r = 0; r < repeats; r++) {
                listA[r] = init_list(size);
                listB[r] = init_list(1000);
            }
            clock_gettime(CLOCK_MONOTONIC, &t_start);
            for (int r = 0; r < repeats; r++) {
                append(listB[r], listA[r]);
            }
        ...
        printf("%d\t%0.5f\n", size, (double) min / repeats / 1000.0);
    }
}


int main() {
    bench(5, 500, 100000, 2500);
    return 0;
}
